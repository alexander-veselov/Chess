...
Application::Application(const Specification& specification)
  ...
  if (!glfwInit()) {
    printf("Could not initalize GLFW\n");
    return;
  }
  ...
  window_handle_ =
      glfwCreateWindow(specification.width, specification.height,
                       specification.name.c_str(), nullptr, nullptr);
  ...
  VkSurfaceKHR surface;
  VkResult err = glfwCreateWindowSurface(g_Instance, window_handle_,
                                         g_Allocator, &surface);
  check_vk_result(err);
